...
@Model.register("qasper_mbr_calibrator")
class QasperMbrCalibrator(Model):
    def __init__(
        self,
        vocab: Vocabulary,
    ...
    def forward(
        ...
        if self._greedy_eval and not self.training:
            ...
            # MBR loss computation
            model_probs = [torch.exp(score) for score in model_scores]
            normalized_model_probs = [p/sum(model_probs) for p in model_probs]
            expected_f1_score = sum([prob * f1 for prob, f1 in zip(normalized_model_probs, f1_scores)])
            self._expected_f1(expected_f1_score)
            mbr_loss = 1 - expected_f1_score
            if mle_loss is None:
                loss = mbr_loss
            else:
                loss = (self._mle_loss_weight * mle_loss) + (self._mbr_loss_weight * mbr_loss)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        return {
            ...
            "expected_f1_score": self._expected_f1.get_metric(reset),
        }
